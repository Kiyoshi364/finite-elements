...
export fe_setup_AB, fe_setup_c0, fe_step
...
function build_vec(f, t0_half, h, N_e, EQoLG;
    gauss_n = 5,
)
    F = fill(0.0, (N_e,))
    for e in 1:N_e
        F_e = build_small_vec(x -> f(x, t0_half), h, e,
            gauss_n=gauss_n,
        )
        _1 = EQoLG[1, e]
        _2 = EQoLG[2, e]
...
function finite_elements(f, u0, T, tau, alpha, beta, gamma, h, N_e)
    ...
    A, B = fe_setup_AB(f, alpha, beta, gamma, tau, h, N_e, EQoLG)
...
function fe_setup_AB(f, alpha, beta, gamma, tau, h, N_e, EQoLG)
...
function fe_setup_c0(u0, h, N_e, EQ)
    xs = n_points_from_to(N_e,
        i_start=0, i_end=N_e+1
    )
...
function fe_step(f, A, B, c0, t0, tau, h, N_e, EQoLG)
    ...
    F = build_vec(f, t0_half, h, N_e, EQoLG)
