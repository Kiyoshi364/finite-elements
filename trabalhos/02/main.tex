...
\[ \begin{cases}
    \left[ \mathbb{M} {+} \frac\tau2 \mathbb{K} \right] \; C^{i+1}
    = \tau \; \mathbb{F}\left( t_{i+\frac12} \right)
    - \tau \; \mathbb{G}\left( \frac{3 \; C^i - C^{i-1}}{2} \right)
    + \left[ \mathbb{M} {-} \frac\tau2 \mathbb{K} \right] \; C^i
        &\text{, } i \in \{ 1, \dots, \lfloor \frac{T}{\tau} \rfloor \}
    \\
    C^0 = \; ???
    \\
    C^1 = \; ???
...
Para aproximar o \(C^0\),
vamos usar uma das 4 aproximações da Seção~\ref{sec:U0}.

Para aproximar o \(C^1\),
vamos usar um método do tipo preditor/corretor:

\[ \begin{cases}
    \left[ \mathbb{M} + \frac\tau2 \mathbb{K} \right] \; \tilde{C}^1
    = \tau \; \mathbb{F}\left( t_{\frac12} \right)
    - \tau \; \mathbb{G}\left( C^0 \right)
    + \left[ \mathbb{M} - \frac\tau2 \mathbb{K} \right] \; C^0
    \\
    \left[ \mathbb{M} + \frac\tau2 \mathbb{K} \right] \; C^1
    = \tau \; \mathbb{F}\left( t_{\frac12} \right)
    - \tau \; \mathbb{G}\left( \frac{\tilde{C}^1 + C^0}{2} \right)
    + \left[ \mathbb{M} - \frac\tau2 \mathbb{K} \right] \; C^0
\end{cases} \]
